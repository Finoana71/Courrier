...
namespace CourrierFront.Services
{
    ...
    {
        public void ExportCourriersToPdf(List<Courrier> list, Stream outputStream)
        {
        ...
        private void AddCourriersTable(Document document, List<Courrier> list)
        {
        ...
        private void AddTableHeaders(PdfPTable table)
        {
            ...
            var cellBackgroundColor = new iTextSharp.text.Color(51, 102, 100);
        ...
        private void AddTableData(PdfPTable table, IEnumerable<Courrier> courriers)
        {
            var dataFont = FontFactory.GetFont(FontFactory.HELVETICA, 10, iTextSharp.text.Color.BLACK);

            foreach (var courrier in courriers)
            {
                table.AddCell(new PdfPCell(new Phrase(courrier.Objet, dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Expediteur, dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Reference, dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Departement.Nom, dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Fichier != null ? "Oui" : "Non", dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.DateCreation.ToShortDateString(), dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Flag.Libelle, dataFont)));
                table.AddCell(new PdfPCell(new Phrase(courrier.Interne ? "Non" : "Oui", dataFont)));
